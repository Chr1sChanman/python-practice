...
**Helpful suggestion:** When refactoring weights into `ScoringConfig` (Phase 1), the AI proposed making the `config` parameter **optional with a default of `None` → `ScoringConfig()`** rather than a required kwarg. My thought process prior to the AI of making it required would have broken all 7 existing tests and forced a multitude of test edits before even verifying the refactor. The optional matching defaults pattern lets me implement the change with zero test edits, which made the next implemention easier and safer.
